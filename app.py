...
def visualize_confidence_level(prediction_proba):
    ...
    ax = grad_percentage.plot(kind='barh', figsize=(7, 4), color='#f63366', zorder=10, width=0.5)
    ...
    vals = ax.get_xticks()
    for tick in vals:
        ax.axvline(x=tick, linestyle='dashed', alpha=0.4, color='#f63366', zorder=1)
...
st.image(image, caption='wine',use_column_width=True)
